...
//   - GET  /auth/lwa/device/callback -> BindPairing (called once the
//     browser leg's LWA code-exchange + two-check Validate has already run;
//     that HTTP glue belongs to the web-core/auth route handler, not here)
...
//     it to status "pending", 15-minute TTL).
//
//  2. A human completes LWA sign-in in a phone/laptop browser, landing on
//     GET /auth/lwa/device/callback with state=<nonce>. That handler calls
//     BindPairing, which re-runs the exact same Authorize gate every other
//     sign-in surface goes through (owner-binds-first, else owner match or
//     allowlist, else ErrNotAllowed) before doing anything device-specific.
//     Only on success does it create the permanent DEVICE# record
//     (provisioning IoT via the ProvisionIoT hook, minting a fresh FamilyID
//     that will anchor the device's whole 10-year credential lineage) and
//     atomically flip the PAIR row pending -> bound, recording deviceId +
//     userId on it.
//
...
// generated on-chip. The nonce is what the device encodes into the
// QR/pairing-code a human scans to complete the browser leg.
func RegisterPairing(ctx context.Context, st *store.Store, codeChallenge string) (nonce string, expiresAt time.Time, err error) {
	if st == nil {
	...
	return nonce, time.Unix(pair.TTL, 0).UTC(), nil
}
...
// that couldn't sign in through the web/Android flow either. Only on
// success does it create the permanent DEVICE# record (FamilyID fresh,
// anchoring the whole 10-year credential lineage this device will ever
// have) and provision IoT (ProvisionIoT). The PAIR row is atomically
...
func BindPairing(ctx context.Context, st *store.Store, log *slog.Logger, nonce, deviceName string, profile *LWAProfile) error {
	if st == nil {
