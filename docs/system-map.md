...
internal/memory: per-user `ENT#<type>#<id>` + `EMB#<id>` items, Titan v2 (512 dims) query
embeddings, in-partition cosine — Query/GetItem only, never Scan. Since 2026-09-14
(`agentcore-memory`, dual running) the sink also writes one AgentCore Memory event per exchange
(`internal/agentmemory`, gate `AGENTCORE_MEMORY_MODE`), the broker preloads top records into a
`REMEMBERED` block at mint, `memory_search` returns them under `remembered`, purge deletes them.
A "memory not wired" RCA checks `AGENTCORE_MEMORY_ID`/`_MODE` as well as `Deps.Memory`.
...
principal and still gates every launch through its capability matrix and audit log. Three cross-repo
...
and no summary email fires; the invoke task discriminator must match or every call 401s; the
preprocess job statuses are lowercase `pending`/`done`/`error` (`internal/ghost`) — a mismatch
discards every finished rewrite and reports it to the owner as a timeout that never happened. Only
the owner's instructions reach Opus — token, deploy gate and directive are appended afterwards.
