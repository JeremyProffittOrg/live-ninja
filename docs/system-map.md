...
internal/memory: per-user `ENT#<type>#<id>` items for people, places, projects, tasks, and
plans — Query/GetItem only, never Scan. `memory_search` matches that text. Since 2026-09-14
conversation facts go to AgentCore Memory (`internal/agentmemory`, gate `AGENTCORE_MEMORY_MODE`).
The broker preloads top records into a `REMEMBERED` block at mint, `memory_search` returns them
under `remembered`, and purge deletes them. A "memory not wired" RCA checks
`AGENTCORE_MEMORY_ID`/`_MODE` as well as `Deps.Memory`.
