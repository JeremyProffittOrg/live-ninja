...
- [ ] `emb-retire` — after two weeks of dual running with the smoke set passing, remove `EMB#` writes, `ListEmbeddings`, the cosine path and the Titan IAM statement; `contracts/api.md:209-211` and `PRD.md` memory sections rewritten to describe AgentCore; `MaxEmbeddingsPerUser` deleted. Done when: `grep -rn "EMB#\|Cosine(" internal/ cmd/` prints nothing and `go test ./...` passes.
