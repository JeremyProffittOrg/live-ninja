...
> 7. `[~]` **Nova Sonic:** broker JWT_KMS_KEY_ID + kms:Sign fix DEPLOYED (`3ec9e45` — bootstrap now 200 w/ bridge session; the 502 nova_bridge_unavailable is gone); CloudFront WS pass-through verified (token-less probe → bridge 401). REMAINING: client hangs "Connecting…" with nothing bridge-side — token-transport mismatch between realtime.mjs's WS open and the bridge's expected auth (browser WS can't send headers); debug agent tracing both sides now, incl. missing error propagation that leaves the UI stuck in Connecting.
>
