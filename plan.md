...
>    11. `[x]` **SDIO stability campaign (afternoon round 2) — crashes ELIMINATED, verified turn on "Hi Lily":** the `sdio_rx_get_buffer`/`sdio_push_data_to_queue` asserts persisted even on slave v1.4.7 — root cause is INTERNAL DMA heap exhaustion during session setup (observed: even a 2 KB internal alloc failing). Fix chain, all HIL-verified in the final run (wake → mint → WSS → question → thinking → SPEAKING answer → back to listening, ZERO crashes): (a) **local patches to the vendored esp-hosted host driver** (`managed_components/espressif__esp_hosted`, marked "LIVE NINJA LOCAL PATCH — keep across component updates"): streaming RX burst buffer falls back to PSRAM instead of asserting AND self-heals back to a 16 KB internal buffer once bursts subside (a sticky PSRAM buffer slowed every SDIO read → ~40% uplink loss — found via the patch's own logs); mempool packet-buffer alloc falls back to PSRAM on internal exhaustion, with `mempool_free` releasing PSRAM buffers for real so the reuse pool (a potential TX DMA source) stays internal. (b) `LN_RT_SEND_TIMEOUT` 1 s → 4 s: the SDIO TX path stalls transiently 1-2 s under combined load and the 1 s write timeout made esp_websocket_client abort the whole session on a survivable hiccup (this was THE chronic link-dropper, present even in the first working build); the decoupled uplink task absorbs the block. (c) `RX_MAX_SIZE` SDIO mode was tried and REVERTED — it broke TLS connects outright (repeated ESP_ERR_HTTP_CONNECT) and ate 28 KB internal for its pool; streaming + patches is the keeper. **Dead-end knowledge:** a WSS link drop mid-turn re-mints into a BLANK OpenAI session, so the in-flight question is orphaned and Thinking times out — riding through stalls (b) matters more than fast reconnect. Deeper root cause for someday: what exhausts internal DMA heap during session setup (lwIP TCP buffers + esp-tls internals + AFE + fragmentation suspected).
>
