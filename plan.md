...
> **RESUME STATE — checkpoint 2026-07-18 ~08:15 EDT. HEAD `6306db2` + one uncommitted CSS change (ptt 2x — commit it first). Deploy run 29643496041 (bucket CORS + toast detail + first-candidate ICE) was IN FLIGHT at checkpoint — verify it landed. This supersedes the 04:30 block below (still valid background). The owner is ACTIVELY live-testing and firing rapid requests — expect mid-turn messages.**
>
...
> 1. **Hands-free start "Failed to fetch" — fix deployed, VERIFY.** Root cause: the trained-model download is a cross-origin presigned-S3 fetch and the wakewords bucket had NO CORS config (not CSP — no violation event fired). `6306db2` adds GET/HEAD CORS for live.jeremy.ninja to WakewordsBucket. After the deploy: reload /conversation twice, toggle hands-free ON → engine must reach 'listening' with phrase "hey live ninja". Debug recipe that found it: `import('/static/js/wakeword.mjs')` in the page console → `createWakeWordEngine({wakeWordId:'hey-live-ninja'}).start()` and catch. The failure toast now shows the underlying error under Details (owner rule: ALWAYS surface the real error, never a bare "couldn't").
> 2. **Speaker-driven e2e voice test (owner directive: USE THE PC AUDIO).** PowerShell TTS (`Add-Type -AssemblyName System.Speech; (New-Object System.Speech.Synthesis.SpeechSynthesizer).Speak('...')`) plays through the PC speakers; Chrome's AEC does NOT cancel other apps' output, so the site's mic hears it as real speech. Test: start a session (or hands-free wake once #1 verified) → TTS "Hey Live Ninja" (wake) and a question → verify user transcript renders (transcription now enabled), assistant replies with audio, volume graph moves. This closes the last unverified web-voice link. PC audio devices: Realtek = normal speaker, "USB Audio" = designated test speaker (plan §HIL).
> 3. **Theming (task #13, owner spec):** mic (ptt) button 2x — DONE in the working tree (168px, font 60 — COMMIT IT). Remaining: extensive color/style customization adjustable from Settings — pull ideas from podcasts.jeremy.ninja (WebFetch it); implement as an `appearance` settings object (schema+validation+UI) applied at runtime via the existing `--ln-*` CSS custom properties (app.css tokens are already var-driven); **default look = HAL 9000 on the left rail** (red glowing eye orb on black — restyle .ln-orb for a hal9000 theme and make it the default orbTheme). Non-trivial UI → run the mandatory multi-persona design pass first (spec: fields/types/defaults per machine rules).
...
> 5. Fallback-turn tool-calling (typed messages with no live session say "I can't create files" — text-only fallback has no tools; candidate feature).
> 6. Pre-existing plan items: Nova Sonic re-enable (isolated deploy), M8 launch pass (SNS confirm click, Play listing, runbook, go/no-go), device-pairing user_code (RFC 8628, blocks device launch), PWA icon svg.
>
