...
> 7. **NEW (owner, ~08:40): Tab5 onboarding rework — WiFi options as slide-out panels on the main screen** instead of separate full-screen views (harness Task #1; also folds in 4 findings from the Tab5 UI documentation pass: Settings body sized 720-72 instead of 1280-72 (~560px undersized), error-screen auto-retry countdown built but never wired from ln_ctrl.c, stale "1280x720 landscape" header comment in ln_ui.h, and no touch path into Listening from Idle — add tap-the-orb via `ln_wake_trigger()`). Launching next.
>
