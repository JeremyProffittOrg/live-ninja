...
> **RESUME STATE — checkpoint 2026-07-18 ~12:15 EDT. HEAD `10d1799`, working tree CLEAN, all deploys green (last: 29651226164). Supersedes the 08:15 block below (whose OPEN ITEMS are annotated ✅ in place; its session-ops notes remain must-reads). The owner is actively live-testing — expect mid-turn messages.**
>
...
> 5. **M8 launch pass:** SNS ops-topic confirm click (OWNER — still pending); Android signed release APK + `assetlinks.json` + Play listing (release keystore `C:\dev\live-ninja-keys\release.keystore`, held by owner); M5 firmware release channel; runbook; go/no-go against the risk tables. Android has also NEVER run on a device/emulator (M4 DoD gap).
...
> 8. `[~]` **Batch-6 IN FLIGHT** (wf_b1312d39): persona-centric refactor — personaPrefs {voice, accent} embedded per persona + pop-up persona editor (dialog partial + personaeditor.mjs); conversation shell redesign: header REMOVED, state pill stacked over cost in rail top-right, persona dropdown + Edit under the mic (HR-separated), History/Memory buttons, docked settings slide-out drawer, mic sensitivity chips (Low/Med/High) under Test-my-mic (Tasks #5+#6 subsumed). **After integration: curate each built-in's best voice + default accent (owner ask — e.g. Josh male/verse not marin, butler+bard→british, sommelier→french, grandma→sage).**
> 9. `[ ]` **Task #7 queued** (after batch-6): raw LLM transcripts in history detail; per-session cost persisted on CONV + shown in history list/detail; "long conversations" (>8 turns) filter button.
> 10. Open questions for owner: Speed/Energy/Register voice knobs (proposed, unpicked); first-words-during-Connecting are lost to the model entirely — wake-buffer replay is the full fix if prefetch isn't enough.
> 11. Ops notes this wave: two "cancelled" runs were GitHub queue-replacement (newer push absorbs the queued one) — not failures; pipeline-monitor hook fires /fix-pipeline advice on them, ignore for cancellations. Selective-commit discipline: agents' shared-file test fakes must ride in the same commit (CI vet catch). plan.md updates: owner asked for FREQUENT updates — refresh at every batch integration.
>
